...
type Func func(invocation.Context) ([]*yaml.Node, error)

// Eval calls the function
func (f Func) Eval(ctx invocation.Context) ([]*yaml.Node, error) {
	return f(ctx)
}
...
func Empty() expr.Expr {
	return Func(func(invocation.Context) ([]*yaml.Node, error) {
		return nil, nil
	})
}
