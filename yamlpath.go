...
func Compile(path string, opts ...Option) (*YAMLPath, error) {
	cfg := &compile.Config{
		Table: tableV1().New(),
	}
	for _, opt := range opts {
		if err := opt.apply(cfg); err != nil {
			return nil, fmt.Errorf("yamlpath: %w", err)
		}
	}
...
func MustCompile(str string, opts ...Option) *YAMLPath {
	yp, err := Compile(str, opts...)
	if err != nil {
		panic(err)
	}
	return yp
